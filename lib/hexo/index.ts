...
class Hexo extends EventEmitter {
  ...
  constructor(base = process.cwd(),  args: Args = {}, root = "") {
    super();

    this.base_dir = base + sep;
    this.root_dir = base + sep;
    if (root.length > 0)
    {
      this.root_dir = root + sep;
    }
    ...
    //
    this.plugin_dir = join(this.root_dir, 'node_modules') + sep;
